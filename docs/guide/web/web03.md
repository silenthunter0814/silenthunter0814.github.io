...
- `^` : 字符表示字符串的开头。 它是一个锚点，可防止 exec 跳过非类似 URL 的前缀。
- `(?:([A-Za-z]+):)?` : 此因子与 scheme 名称匹配，但前提是其后跟有 `:`（冒号）。 `(?:...)` 表示非捕获组。 后缀 `？` 表示该组是可选的。  
  这意味着重复零次或一次。 `(...)` 表示捕获组。 捕获组复制它匹配的文本并将其放入结果数组中。 每个捕获组都有一个编号。 第一个捕获组是 `1`，因此与该捕获组匹配的文本的副本将出现在 `result[1]` 中。 `[...]` 表示字符类。 此字符类 `A-Za-z` 包含 26 个大写字母和 26 个小写字母。 连字符表示从 A 到 Z 的范围。后缀 `+` 表示字符类将匹配一次或多次。 该组后面跟着 `:` 字符，它将按字面匹配。
- `(\/{0,3})` : 下一个因子是捕获组 `2`。`\/` 表示应匹配 `/`（斜杠）字符。 它用 `\`（反斜杠）转义，这样就不会被误解为正则表达式文字的结尾。 后缀 `{0,3}` 表示 `/` 将匹配 0 或 1 或 2 或 3 次。
- `([0-9.\-A-Za-z]+)` : 下一个因子是捕获组 `3`。它将匹配由一个或多个数字、字母或 `.` 或 `-` 组成的主机名。 `-` 被转义为 `\-` 以防止与范围连字符混淆。
- `(?::(\d+))?` : 下一个因子可以选择匹配端口号，该端口号是前面带有 `:` 的一个或多个数字的序列。 `\d` 代表数字字符。 一系列的一个或多个数字将捕获组 `4`。
- `(?:\/([^?#]*))?` : 我们还有另一个可选组。 这个以 `\` 开头。 字符类 `[^?#]` 以 `^` 开头，表示该类包含除 `?` 和 `＃` 之外的所有字符。 `*` 表示字符类匹配零次或多次。  
  请注意，我在这里很草率。 除了 `?` 和 `#` 之外的所有字符的类别 包括行结束字符、控制字符和许多其他不应该在此处匹配的字符。 大多数时候，这会达到我们想要的效果，但存在一些错误文本可能漏掉的风险。 草率的正则表达式是安全漏洞的一个常见来源。 编写草率的正则表达式比编写严格的正则表达式容易得多。
- `(?:\?([^#]*))?` : 接下来，我们有一个以 `?` 开头的可选组。 它包含捕获组 `6`，其中包含零个或多个非 `#` 字符。
- `(?:#(.*))?` : 我们有一个以 `#` 开头的最后一个可选组。 这 `.` 将匹配除行结束字符之外的任何字符。
- `$` : 代表字符串的结尾。 它向我们保证 URL 末尾后没有多余的内容。

这些是正则表达式 parse_url 的因子。

可以制作比 `parse_url` 更复杂的正则表达式，但我不推荐它。 正则表达式在简短时效果最好。 只有这样，我们才能确信它们正常工作，并且在必要时可以成功修改它们。
...
- `/^` :
- `$/i` :  
我们再次使用 `^` 和 `$` 来锚定正则表达式。 这会导致文本中的所有字符都与正则表达式匹配。 如果我们省略了锚点，则正则表达式会告诉我们字符串是否包含数字。 通过锚点，它可以告诉我们字符串是否只包含数字。 如果我们只包含 `^`，它将匹配以数字开头的字符串。 如果我们只包含 `$`，它将匹配以数字结尾的字符串。  
...
- `(?:\.\d*)?` :  
表示可选的非捕获组。 该组将匹配小数点后跟零个或多个数字。

- `(?:e[+\-]?\d+)?` :  
这是另一个可选的非捕获组。 它匹配 `e`或 (`E`)、可选正负符号和一个或多个数字。
