...
在任何函数之外用 var 初始化的变量都会分配给全局对象，具有全局作用域并且在任何地方都可见。 在函数内部用 var 初始化的变量被分配给该函数，它是本地的，并且仅在函数内部可见，就像函数参数一样。

定义到与全局变量同名的函数中的任何变量都优先于全局变量，从而遮蔽它。

重要的是要理解块（由一对花括号标识）不会定义新的作用域。 只有在创建函数时才会创建新的作用域，因为 var 没有块作用域，而是函数作用域。
