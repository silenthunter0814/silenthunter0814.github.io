...
无论将超时设置为 0 秒还是 5 分钟，都没有什么区别——异步代码调用的 console.log 将在同步顶级函数之后执行。 发生这种情况是因为 JavaScript 主机环境（在本例中为浏览器）使用称为事件循环的概念来处理并发或并行事件。 由于 JavaScript 一次只能执行一条语句，因此它需要通知事件循环何时执行哪条特定语句。 

事件循环使用堆栈和队列的概念来处理此问题。
